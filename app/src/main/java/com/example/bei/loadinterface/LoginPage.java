        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                if (username.equals(mUserName)) {
                    Toast.makeText(LoginPage.this, "账号已被注册", Toast.LENGTH_SHORT).show();
                    isSignUp = true;
                    break;
                } else {
                    isSignUp = false;
...
// 核对账号信息并登录
    private void singIn() {
        ...
        if (cursor.moveToFirst()) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (username.equals(mUserName) && password.equals(mPassword)) {
                    ...
                    Toast.makeText(LoginPage.this, "账号已登录", Toast.LENGTH_SHORT).show();
                    editor = pref.edit();
                    // 检查复选框是否被选中
        ...
        if (!isSignIn) {
            Toast.makeText(LoginPage.this, "账号或密码错误，请重新输入", Toast.LENGTH_SHORT).show();
        }
    }
//  更新登录状态
